...
bool CPlayerObject::Init()
{
    ...
    mScene->GetInput()->AddBindKey("SkillQ", 'Q');
    ...
    mScene->GetInput()->AddBindFunction<CPlayerObject>("SkillQ",
        EInputType::Down, this, &CPlayerObject::SkillQ);
...
void CPlayerObject::SkillQ(float DeltaTime)
{
    /*
        1. 스킬 사용 시 사각형 발사
        2. 일정 거리 이동 후 멈춤
    ...
    */
    ...
    CLog::PrintLog("Q Skill Active");

    CFloorDotBullet* Bullet = mScene->CreateObj<CFloorDotBullet>("Bullet");

    Bullet->SetBulletCollisionProfile("PlayerAttack");
    Bullet->SetBulletType(EFloorType::Floor);

    FVector3D Pos = mRoot->GetWorldPosition();
    FVector3D Dir = mRoot->GetAxis(EAxis::Y);

    Bullet->SetWorldScale(400.f, 400.f);
    Bullet->SetWorldRotation(mRoot->GetWorldRotation());
